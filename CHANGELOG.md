...
- After CJK text, `.`, `,`, `:`, `;`, `)`, `]`, `%` or `"` now stays on the same line as a following word or number, as browsers do. In `甲乙丙.first_week_voltage}户`, lines no longer break after the period (#274).
