...
- After CJK text, lines no longer start with punctuation such as `'`, `/`, `|`, `‼` or `％`. A word or number after `!`, `}`, `/` or `|` there now stays with the mark as each browser keeps it: Chrome keeps ASCII letters and digits, Safari keeps digits, and letters after `}` that follows an ideograph or Hangul syllable, and Firefox keeps only digits after `/`.
