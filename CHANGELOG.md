...
- In `pre-wrap`, a line that wraps no longer counts the spaces and tabs it ends on in its width, and spaces and tabs before a newline or at the end of the text count only as far as they fit, so boxes sized from `measureLineStats()` or `walkLineRanges()` no longer run past the text. A run of spaces and tabs at the end of a line now stays on that line whole, instead of starting the next line with a tab or a space after a tab.
