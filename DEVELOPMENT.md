...
through LaunchServices (`open -n -g -a`) and stops the process that names its
disposable profile, because macOS 27 denies a shell's processes access to apps'
folders under `~/Library/Application Support` and a directly spawned Firefox
exits with "Could not find profile folder." for any `--profile`.
