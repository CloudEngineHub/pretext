...
- Firefox keeps a date such as `2025-08-01` whole, where Chrome and Safari break after its hyphens. Pretext splits it for every engine, so #225's first reproduction is only observed in Firefox. Firefox also keeps a hyphen with the digits after it in `crash-log-2026-09-12.txt`, which it breaks only after `crash-`, where Pretext breaks at `log-|2026` or `09-|12`. UAX #14 LB25 forbids a break between a hyphen and a following number.
- Firefox breaks after `/` before a letter, as in `https://|example.com`, `example.com/|docs`, `src/|layout.ts` and `and/|or`, where Pretext keeps the unit whole; Chrome's lines match Pretext on those shapes. `/` isn't in Gecko's table of ASCII characters that never break, so ICU4X decides. Text split across rich-inline items next to `/`, such as links, paths and `and` + `/or`, loses Firefox heights that main matched by analyzing each item alone and breaking after `/`. Trace digits after `/`, and whether Firefox still keeps a URL query after `?` together, then model it in the Gecko profile with its own installed full-suite Firefox sweep.
