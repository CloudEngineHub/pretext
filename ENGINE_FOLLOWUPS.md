...
- After Latin letters, Pretext still allows a break before closing punctuation (CL, CP, EX, IS) and NS such as `，`, `」` or `：`, which UAX #14 forbids (LB13, LB21). Numeric runs already keep it. #245 tried attaching them: it created kinsoku units that got no emergency breaks, so it lost 42 Chrome, 52 Safari and 52 Firefox LTR rows, mostly shapes like `739x「value」! end`. With emergency breaks inside kinsoku units, `abc」。d` still differs: at 26px Safari and Firefox give `ab / c」 / 。 / d`, where Pretext gives `ab / c / 」 / 。 / d`.
...
- Firefox breaks after `/` before a letter, as in `https://|example.com`, `example.com/|docs`, `src/|layout.ts` and `and/|or`, where Pretext keeps the unit whole; Chrome's lines match Pretext on those shapes. `/` isn't in Gecko's table of ASCII characters that never break, so ICU4X decides. Text split across rich-inline items next to `/`, such as links, paths and `and` + `/or`, loses Firefox heights that main matched by analyzing each item alone and breaking after `/`. Before a Latin-1, Cyrillic or CJK letter all three browsers break after `/`, and none breaks after `/` before a digit, as in `1/2` or `example.com/2026`. Firefox breaks after `?` before a URL query, as Chrome and Safari do, and inside the query after `/` before a letter. Since the Gecko profile keeps `8:30-4:30` whole, the mixed-app-text corpus takes one line more than Firefox at 325-350px, where Firefox breaks the URL after a `/` before a letter (`backup URL https://example.com/ | reports/q3?…`, or `…/reports/ | q3?…` from 345px) and Pretext moves the URL to the next line; main matched only by breaking after `8:30-`. Model it in the Gecko profile with its own installed full-suite Firefox sweep.
...
- In engines Pretext doesn't recognize, rich-inline layout breaks at every item boundary, and it fills the rest of a line with an item's first segment when that segment takes emergency breaks, instead of wrapping at the boundary: items `漢` and `丙.first` give `漢丙.fir` / `st`, though `丙.first` fits the next line. CJK units such as `字。` in items `漢` and `字。字` reach this too, since they take emergency breaks.
